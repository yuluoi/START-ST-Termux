...
toggle_notification_submenu() {
    ...
    if [ "$new_status" == "true" ]; then
        enable_notification_keepalive="true"; echo "true" > "$config_file"; echo "✅ 通知保活功能已 [开启] 并已保存。"
    elif [ "$new_status" == "false" ]; then
        enable_notification_keepalive="false"; echo "false" > "$config_file"; echo "✅ 通知保活功能已 [关闭] 并已保存。"
    else
        echo "无效输入，设置未改变。"
    fi; sleep 2
}
        ...
        case "$action_choice" in
            ...
            "d"|"D")
                if [ "$pkg_name" == "termux-api" ]; then
                    if command -v termux-open-url >/dev/null; then echo "正在浏览器中打开下载页面..."; termux-open-url "$termux_api_apk_url"; sleep 2
...
package_selection_submenu() {
    while true; do
        ...
        case "$pkg_choice" in
            1) package_manager_submenu "git" "git" true;; 2) package_manager_submenu "curl" "curl" true;; 3) package_manager_submenu "nodejs-lts" "node" true;; 4) package_manager_submenu "jq" "jq" false;; 5) package_manager_submenu "termux-api" "termux-wake-lock" false;; 0) break;; *) echo "无效选择..."; sleep 1; continue;;
        esac
    done
}
termux_setup() {
    ...
    if [ "$confirm" == "y" ]; then
        echo; echo "--- [步骤 1/2] 正在更新 Termux 基础包 ---"; yes | pkg upgrade; echo "--- [步骤 2/2] 正在安装核心软件包 ---"; apt update && apt install git curl nodejs-lts -y; echo "✅ 环境初始化完成！"; sleep 2
...
update_submenu() {
    ...
    if [ -z "$latest_ver" ] || [ "$latest_ver" == "获取失败" ]; then echo "  ❌ 未能获取最新版本信息..."; echo; echo "========================================="; read -n 1 -p "按任意键返回..."; return; fi
    if [ "$local_ver" == "$latest_ver" ] && [ "$local_ver" != "未知" ]; then echo "  ✅ 已是最新版本。"; echo; echo "========================================="; read -n 1 -p "按任意键返回..."; return; fi
    prompt_text="发现新版本！"; [ "$local_ver" == "未知" ] && prompt_text="SillyTavern 尚未安装或无法检查版本(可能未安装jq)。"
    echo "  $prompt_text"; echo "========================================="; echo; echo "   [1] 立即下载/更新"; echo; echo "   [2] 暂不操作"; echo; echo "========================================="
