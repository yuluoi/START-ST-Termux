...
if [ -f "$config_file" ]; then
    ...
    enable_notification_keepalive="true"; echo "$enable_notification_keepalive" > "$config_file"
fi
...
start_llm_proxy() {
    ...
    if [ ! -d "$llm_proxy_dir" ]; then
        err "LLM代理服务目录 '$llm_proxy_dir' 不存在！"; return 1
    fi
    if [ ! -f "$start_script_path" ]; then
        err "启动脚本 '$start_script_path' 未找到！"; return 1
    fi
    echo "进入启动目录并执行..."
    (cd "$(dirname "$start_script_path")" && chmod +x start-termux.sh && ./start-termux.sh start)
    if [ $? -ne 0 ]; then
        err "LLM 代理服务启动失败或异常退出。"
    else
        echo "✅ LLM 代理服务已停止。"
    fi
    read -n 1 -p "按任意键返回主菜单..."
}
toggle_notification_submenu() { clear; echo "========================================="; echo "           通知保活功能设置            "; echo "========================================="; echo; echo "  此功能通过创建一个常驻通知来增强后台保活。"; echo "  当前状态: $enable_notification_keepalive"; echo; echo "========================================="; read -p "请输入 'true' 或 'false' 来修改设置: " new_status; if [ "$new_status" == "true" ]; then enable_notification_keepalive="true"; echo "true" > "$config_file"; echo "✅ 通知保活功能已 [开启] 并已保存。"; elif [ "$new_status" == "false" ]; then enable_notification_keepalive="false"; echo "false" > "$config_file"; echo "✅ 通知保活功能已 [关闭] 并已保存。"; else echo "无效输入，设置未改变。"; fi; sleep 2; }
...
# ============================ [区块] 脚本主程序入口 ============================
# 【重要】设置一个陷阱，无论脚本如何退出，都执行 cleanup 函数
trap cleanup EXIT
...
while true; do
    ...
    echo; echo "   [1] 🟢 启动 SillyTavern $keepalive_status_text"; echo; echo "   [2] 🟢 启动LLM代理服务"; echo; echo "   [3] 🔄 (首次)安装 / 检查更新 SillyTavern"; echo; echo "   [4] 📦 软件包管理"; echo; echo "   [5] ⚙️  进入(可选的)原版脚本菜单"; echo; echo "   [6] 🚀 Termux 环境初始化"; echo; echo "   [7] 🔔 通知保活设置 (当前: $enable_notification_keepalive)"; echo; echo "   [0] ❌ 退出到 Termux 命令行"; echo; echo "========================================="

    choice=""
    prompt_text="请按键选择 [1-7, 0] "
    final_text="秒后自动选1): "
    ...
    case "${choice:-1}" in
        1)
            ...
            echo "SillyTavern 已启动 (PID: $st_pid)，状态文件已创建。"
        ...
        5)
            if [ ! -f "$install_script_name" ]; then
                ...
                echo "选择 [5]，正在进入原版脚本菜单..."; sleep 1; clear; ./"$install_script_name"; break
            fi
            ;;
        6) termux_setup;;
        7) toggle_notification_submenu;;
        0)
            echo "选择 [0]，已退回到 Termux 命令行。"; pkill -f "termux-wake-lock" &> /dev/null;
            break
            ;;
