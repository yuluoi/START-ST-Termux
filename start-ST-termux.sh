...
enable_menu_timeout="true" # [新增] 控制主菜单倒计时的开关

# --- [区块] 配置管理 ---
load_config() {
    enable_notification_keepalive="true"; enable_auto_start="true"; enable_password_start="false"; enable_menu_timeout="true"
...
save_config() {
    ...
    echo "enable_menu_timeout=$enable_menu_timeout" >> "$config_file" # [新增] 保存倒计时设置
}
...
update_st_incremental() {
    if [ ! -d "$sillytavern_dir/.git" ]; then
        err "错误：找不到 .git 目录，无法增量更新。"
        return 1
    fi

    # [核心修复] 步骤1: 先备份当前版本，再进行任何修改
    echo "正在创建当前版本的备份..."
    rm -rf "$sillytavern_old_dir"
    cp -r "$sillytavern_dir" "$sillytavern_old_dir" || { err "创建备份失败！请检查磁盘空间和权限。"; return 1; }

    # 步骤2: 重置本地仓库，为更新做准备
    echo "正在重置本地仓库以避免更新冲突..."
    (cd "$sillytavern_dir" && git reset --hard origin/release) || { err "Git 重置失败！请检查您的网络或尝试全新更新。"; return 1; }

    # 步骤3: 执行增量更新
    echo "正在执行增量更新 (git pull)..."
    (cd "$sillytavern_dir" && git pull) || { err "Git 更新失败！即使在重置后也失败，建议使用全新更新。"; return 1; }
    
    # 步骤4: 更新依赖
    echo "正在更新 npm 依赖..."
    (cd "$sillytavern_dir" && npm install) || { err "npm 依赖安装失败！"; return 1; }
...
additional_features_submenu() { while true; do clear; echo "========================================="; echo "                附加功能                 "; echo "========================================="; echo; echo "   [1] 📦 软件包管理"; echo; echo "   [2] 🚀 Termux 环境初始化"; echo; echo "   [3] 🔔 通知保活设置 (当前: $enable_notification_keepalive)"; echo; echo "   [4] ⚡️ 跨会话自启设置 (当前: $enable_auto_start)"; echo; echo "   [5] 🔐 密码启动 (当前: $enable_password_start)"; echo; echo "   [6] ⏳ 开/关主菜单倒计时 (当前: $enable_menu_timeout)"; echo; echo "   [7] ⚙️  进入(可选的)原版脚本菜单"; echo; echo "   [0] ↩️  返回主菜单"; echo; echo "========================================="; read -n 1 -p "请按键选择 [1-7, 0]: " sub_choice; echo; case "$sub_choice" in 1) package_selection_submenu;; 2) termux_setup;; 3) toggle_notification_submenu;; 4) toggle_auto_start_submenu;; 5) toggle_password_start_submenu;; 6) toggle_menu_timeout_submenu;; 7) if [ ! -f "$install_script_name" ]; then clear; echo "========================================="; echo "      ⚠️ $install_script_name 脚本不存在"; echo "========================================="; echo; echo "   [1] 立即下载"; echo; echo "   [2] 暂不下载"; echo; echo "========================================="; read -n 1 -p "请按键选择 [1-2]: " choice; echo; if [ "$choice" == "1" ]; then echo "正在下载 $install_script_name..."; curl -s -O "$install_script_url" && chmod +x "$install_script_name"; if [ $? -eq 0 ]; then echo "下载成功！正在进入..."; sleep 1; clear; ./"$install_script_name"; exit 0; else err "下载失败！"; fi; fi; else echo "选择 [7]，正在进入原版脚本菜单..."; sleep 1; clear; ./"$install_script_name"; exit 0; fi;; 0) break;; *) err "输入错误！请重新选择。";; esac; done; }
